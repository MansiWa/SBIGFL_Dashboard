...

{
    public class DashboardRepository : IDashboardRepository
    {
        ...
        public async Task<IActionResult> GetAll(Dashboard model)
        {
        ...
        public async Task<IActionResult> Credit(Dashboard model)
        {
            using (var connection = _context.CreateConnection())
            {
                //Guid? userIdValue = (Guid)user.UserId;
                try
                {
                    ...
                    var queryResult = await connection.QueryMultipleAsync("proc_CreditDash", SetParameter(model), commandType: CommandType.StoredProcedure);
                    var Model = queryResult.Read<Object>();
                    var outcome = queryResult.ReadSingleOrDefault<Outcome>();
                    var outcomeId = outcome?.OutcomeId ?? 0;
                    var outcomeDetail = outcome?.OutcomeDetail ?? string.Empty;
        ...
        public DynamicParameters SetParameter(Dashboard user)
        {
            ...
            parameters.Add("@client_npa", user.npaString, DbType.String);
            parameters.Add("@products", user.ProductString, DbType.String);
