...
{
    public class PrimeRRateController : Controller
    {
        private readonly HttpClient _httpClient;
        private readonly ILogger<PrimeRRateController> _logger;
        private readonly IStringLocalizer<PrimeRRateController> _localizer;
        public PrimeRRateController(ILogger<PrimeRRateController> logger, IStringLocalizer<PrimeRRateController> localizer, IConfiguration configuration)
        {
        ...
        public IActionResult Index()
        {
            try
            {
                ...
                HttpResponseMessage response = _httpClient.GetAsync(_httpClient.BaseAddress + "/CreditRating/GetAllPRR?user_id=" + new Guid(userid)).Result;
        ...
        public IActionResult Create(CreditRatingModel model)
        {
            try
            {
                ...
                HttpResponseMessage response = _httpClient.PostAsync(_httpClient.BaseAddress + "/CreditRating/InsertPRR", content).Result;
                if (response.IsSuccessStatusCode)
                {
        ...
        public IActionResult Delete(Guid pr_id)
        {
            try
            {
                ...
                string data = JsonConvert.SerializeObject(model);
                StringContent content = new StringContent(data, Encoding.UTF8, "application/json");
                HttpResponseMessage response = _httpClient.PostAsync(_httpClient.BaseAddress + "/CreditRating/DeletePRR", content).Result;
                if (response.IsSuccessStatusCode)
                {
                    dynamic resdata = response.Content.ReadAsStringAsync().Result;
                    dynamic rootObject = JsonConvert.DeserializeObject<object>(resdata);
